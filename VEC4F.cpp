...
VEC4F mul(const VEC4F& u, const VEC4F& v)
{
...
VEC4F VEC4F::conj(void)
{
	VEC4F out(this->data[0], this->data[1], this->data[2], this->data[3]);
	out[1] *= -1;
	out[2] *= -1;
	out[3] *= -1;	
	return out;
...
VEC4F div(const VEC4F& u, const VEC4F& v)
{
	VEC4F v_inv = v;
	v_inv = v_inv.conj() / v_inv.magnitudeSq();
	return mul(u, v_inv);
}
